...
ID3D10Blob* l_pBlob_Errors = NULL;
	...
	hr = D3DX10CreateEffectFromFile( L"SpotLight.fx", NULL, NULL, "fx_4_0", D3D10_SHADER_ENABLE_STRICTNESS | D3D10_SHADER_DEBUG, 0, 
		g_pd3dDevice, NULL, NULL, &g_pEffect,&l_pBlob_Errors, NULL );
...
// Указатель
	...
	g_pTechnique = g_pEffect->GetTechniqueByName( "RenderSpotLight" );
	...
	bd.ByteWidth = sizeof( SimpleVertex ) * 24;// 24 вершины с нормалями
...
void RenderScene()
{
	...
	float LightPos[4] = {0.0f, 0.0f, -3.0f, 1.0f};
	g_pLightPosition->SetFloatVector((float*)&LightPos);

	float LightDirection[4] = {0.0f,0.0f,1.0f,1.0f};
	g_pLightDirection->SetFloatVector((float*)&LightDirection);

	float Phi = 0.9f;// в радианах
	float Theta = 0.4f;// в радианах
