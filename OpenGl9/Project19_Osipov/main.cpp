...
static Posteffect posteffects[posteffectsCount] = {
	...
	{VK_F8,      "data/refl.frag",       0},
	{VK_F9,      "data/sin.frag",        0},
	{VK_F10,     "data/twirl.frag",      0},
	{VK_F11,     "data/sphere.frag",     0},
	{VK_NUMPAD0, "data/ripple.frag",     0},
	{VK_NUMPAD1, "data/contrast.frag",   0},
	{VK_NUMPAD2, "data/binary.frag",     0},
	{VK_NUMPAD3, "data/canny.frag",      0}
};

// пропорция экрана, к которой надо применить фильтр (здесь от 0 до 100, в шейдерах нормируется)
static GLint filterPart = 50;

// параметры фильтров (здесь от 0 до 100, в шейдерах нормируются)
static GLint param1 = 50;
static GLint param2 = 50;

// параметр Canny, показывать ли Собеля
static bool fullCanny = true;
...
void GLWindowRender(const GLWindow &window)
{
	...
	// устанавливаем пропорцию экрана, к которой надо применить фильтр
	glUniform1i(glGetUniformLocation(posteffectProgram, "filterPart"), filterPart);
	// устанавливаем параметры фильтров
	glUniform1i(glGetUniformLocation(posteffectProgram, "param1"), param1);
	glUniform1i(glGetUniformLocation(posteffectProgram, "param2"), param2);
	// если рендерим Canny, передаем параметр, рендерить ли весь метод
	if (posteffectProgram == posteffects[14].program) {
		glUniform1i(glGetUniformLocation(posteffectProgram, "fullCanny"), fullCanny);
		int p1 = param1 - 35;
		if (p1 < 0) {
			p1 = 0;
		}
		glUniform1i(glGetUniformLocation(posteffectProgram, "param1"), p1);
	}
...
void GLWindowInput(const GLWindow &window)
{
	...
	// изменение части экрана, к которой применяется фильтр
	if (InputIsKeyPressed('E')) {
		filterPart += 5;
		if (filterPart > 100) {
			filterPart = 100;
		}
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int)
{
	...
	LoggerCreate("19_OpenGL_8.log");

	if (!GLWindowCreate("19_OpenGL_8. Image Processing. Lev Osipov", 800, 600, false))
		return 1;
