...
void RenderScene(LPVOID lpParam)
{
	...
	spMain.SetUniform("matrices.normalMatrix", glm::transpose(glm::inverse(mModelMatrix)));
	spMain.SetUniform("matrices.modelMatrix", &mModelMatrix);
	spMain.SetUniform("reflParam1", reflParam1);
	spMain.SetUniform("reflParam2", reflParam2);
	spMain.SetUniform("isReflect", 0); // no reflection
	...
	spMain.SetUniform("reflParam1", reflParam1);
	spMain.SetUniform("reflParam2", reflParam2);
	spMain.SetUniform("isReflect", 1); // with reflection
	mdlDeadpool.RenderModel();

	vPos = glm::vec3(180.0f, 0.0f, 50.0f);
	mModelMatrix = glm::translate(glm::mat4(1.0), vPos);
	mModelMatrix = glm::rotate(mModelMatrix, -PI / 2, glm::vec3(0.0f, 1.0f, 0.0f));
	mModelMatrix = glm::scale(mModelMatrix, glm::vec3(1.0f, 1.0, 1.0f));
	spMain.SetUniform("matrices.normalMatrix", glm::transpose(glm::inverse(mModelMatrix)));
	...
	// Controlling reflecion params
	if (Keys::Onekey(VK_LEFT)) {
		reflParam1 -= 10;
		if (reflParam1 < 0) {
			reflParam1 = 0;
		}
	}
	if (Keys::Onekey(VK_RIGHT)) {
		reflParam1 += 10;
		if (reflParam1 > 100) {
			reflParam1 = 100;
		}
	}
	if (Keys::Onekey(VK_DOWN)) {
		reflParam2 -= 10;
		if (reflParam2 < 0) {
			reflParam2 = 0;
		}
	}
	if (Keys::Onekey(VK_UP)) {
		reflParam2 += 10;
		if (reflParam2 > 100) {
			reflParam2 = 100;
		}
