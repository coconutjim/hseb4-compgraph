...
HRESULT InitWindow( HINSTANCE hInstance, int nCmdShow )
{
    ...
    g_hWnd = CreateWindow(
		L"SimpleWindowClass", 
		L"DirectX 10. Проект 2. Лев Осипов", 
		WS_OVERLAPPEDWINDOW,
		CW_USEDEFAULT, 
		CW_USEDEFAULT, 
		WINDOW_WIDTH,
...
HRESULT InitDirect3D10()
{
   ...
   // Свяжем буфер визуализации с графическим конвейером
	...
	hr = D3DX10CreateEffectFromFile( L"CubeText.fx", NULL, NULL,  "fx_4_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, 
	...
	hr = D3DX10GetImageInfoFromFile(L"dx10_tex.bmp", NULL, &InfoFromFile, NULL);
	...
	hr = D3DX10CreateShaderResourceViewFromFile(g_pd3dDevice, L"dx10_tex.bmp", &LoadImageInfo, NULL, &g_pShaderResource, NULL);
	if( FAILED(hr) )
		return hr;
	...
	// Creating the vetrices
	//D3DXVECTOR3 v1 = D3DXVECTOR3(-0.5f, -0.5f, 0.0f);
	//D3DXVECTOR3 v2 = D3DXVECTOR3(0.0f, -0.5f, 0.5f);
	//D3DXVECTOR3 v3 = D3DXVECTOR3(0.5f, -0.5f, 0.0f);
	//D3DXVECTOR3 v4 = D3DXVECTOR3(0.0f, 0.5f, 0.0f);
	D3DXVECTOR3 v1 = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
	D3DXVECTOR3 v2 = D3DXVECTOR3(1.0f, 0.0f, 0.0f);
	D3DXVECTOR3 v3 = D3DXVECTOR3(0.5f, 0.5f * sqrt(3.0f), 0.0f);
	D3DXVECTOR3 v4 = D3DXVECTOR3(0.5f, 1.0f * sqrt(3.0f) / 6, 1.0f * sqrt(6.0f) / 3);

	// Создаем буфер вершин
	SimpleVertex vertices[] =
	{
		v4, D3DXVECTOR4(1.0f, 0.0f, 0.0f, 1.0f),
		v1, D3DXVECTOR4(0.0f, 1.0f, 0.0f, 1.0f),
		v3, D3DXVECTOR4(0.0f, 0.0f, 1.0f, 1.0f),
		v4, D3DXVECTOR4(1.0f, 0.0f, 0.0f, 1.0f),
		v3, D3DXVECTOR4(0.0f, 0.0f, 1.0f, 1.0f),
		v2, D3DXVECTOR4(0.0f, 1.0f, 0.0f, 1.0f),
		v4, D3DXVECTOR4(1.0f, 0.0f, 0.0f, 1.0f),
		v2, D3DXVECTOR4(0.0f, 0.0f, 1.0f, 1.0f),
		v1, D3DXVECTOR4(0.0f, 1.0f, 0.0f, 1.0f),
		v3, D3DXVECTOR4(0.0f, 1.0f, 0.0f, 1.0f),
		v1, D3DXVECTOR4(0.0f, 0.0f, 1.0f, 1.0f),
		v2, D3DXVECTOR4(0.0f, 0.0f, 1.0f, 1.0f)
	};

    D3D10_BUFFER_DESC bd;
    bd.Usage = D3D10_USAGE_DEFAULT;
    bd.ByteWidth = sizeof( SimpleVertex ) * 16;// 16 вершин
 ...
 // Создаем индексный буфер
    ...
    bd.Usage = D3D10_USAGE_DEFAULT;
    bd.ByteWidth = sizeof( DWORD ) * 16; // нужно 16 вершин для 4 треугольников
  ...
  // Инициализируем мировую матрицу
    ...
    D3DXMatrixPerspectiveFovLH( &g_Projection, (float)D3DX_PI / 8, width/(FLOAT)height, 0.1f, 100.0f );
