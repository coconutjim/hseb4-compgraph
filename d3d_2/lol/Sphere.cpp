...
HRESULT InitDirect3D10()
{
   ...
   // Свяжем буфер визуализации с графическим конвейером
	...
	SimpleVertex vertices[258];

	for (int i = 0; i < 258; i++) {
		vertex v = s_vertices[i];
		vertices[i] = { D3DXVECTOR3(v.absolute.x, v.absolute.y, v.absolute.z), D3DXVECTOR2(v.texture.x, v.texture.y) };
	}

    D3D10_BUFFER_DESC bd;
    bd.Usage = D3D10_USAGE_DEFAULT;
    bd.ByteWidth = sizeof( SimpleVertex ) * 258;// 24 вершины
 ...
 // Создаем индексный буфер
    bd.Usage = D3D10_USAGE_DEFAULT;
    bd.ByteWidth = sizeof( DWORD ) * 1440; // нужно 36 вершин для 12 треугольников
